import datetime
import enum
import typing as tp  # noqa
import zoneinfo


class GranularityEnum(enum.Enum):
    """
    Enum for describing granularity
    """
    DAY = datetime.timedelta(days=1)
    TWELVE_HOURS = datetime.timedelta(hours=12)
    HOUR = datetime.timedelta(hours=1)
    THIRTY_MIN = datetime.timedelta(minutes=30)
    FIVE_MIN = datetime.timedelta(minutes=5)


def truncate_to_granularity(dt: datetime.datetime, gtd: GranularityEnum) -> datetime.datetime:
    """
    :param dt: datetime to truncate
    :param gtd: granularity
    :return: resulted datetime
    """
    utc_dt = dt.replace(tzinfo=zoneinfo.ZoneInfo("UTC")).astimezone(tz=zoneinfo.ZoneInfo("UTC"))
    utc_seconds = utc_dt.timestamp()
    utc_trunked = utc_seconds - (utc_seconds % gtd.value.total_seconds())
    ret = (datetime.datetime.fromtimestamp(utc_trunked, tz=zoneinfo.ZoneInfo("UTC")).replace(tzinfo=dt.tzinfo)
           .astimezone(tz=dt.tzinfo))
    if dt.tzinfo is None:
        ret = ret.replace(tzinfo=None)
    return ret


class DtRange:
    def __init__(
            self,
            before: int,
            after: int,
            shift: int,
            gtd: GranularityEnum
    ) -> None:
        """
        :param before: number of datetimes should take before `given datetime`
        :param after: number of datetimes should take after `given datetime`
        :param shift: shift of `given datetime`
        :param gtd: granularity
        """
        self.__before = before
        self.__after = after
        self.__shift = shift
        self.__gtd = gtd

    def __call__(self, dt: datetime.datetime) -> list[datetime.datetime]:
        """
        :param dt: given datetime
        :return: list of datetimes in range
        """
        trunked = truncate_to_granularity(dt, self.__gtd)
        middle = trunked + self.__gtd.value * self.__shift
        current = middle
        for _ in range(self.__before):
            current = current - self.__gtd.value
        ret: list[datetime.datetime] = []
        for _ in range(self.__before + self.__after + 1):
            ret.append(current)
            current = current + self.__gtd.value
        return ret


def get_interval(
        start_time: datetime.datetime,
        end_time: datetime.datetime,
        gtd: GranularityEnum
) -> list[datetime.datetime]:
    """
    :param start_time: start of interval
    :param end_time: end of interval
    :param gtd: granularity
    :return: list of datetimes according to granularity
    """
    cur_time = truncate_to_granularity(start_time, gtd)
    ret: list[datetime.datetime] = []
    while cur_time <= end_time:
        if cur_time >= start_time:
            ret.append(cur_time)
        cur_time = cur_time + gtd.value
    return ret

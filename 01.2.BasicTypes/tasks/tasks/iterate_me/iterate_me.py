...
def get_indices_from_one(elements: list[int]) -> list[int]:
    ...
    """
    return [it + 1 for it, val in enumerate(elements)]
...
def get_max_element_index(elements: list[int]) -> int | None:
    ...
    """
    return None if len(elements) == 0 else elements.index(max(elements))
...
def get_indices(elements: list[int], value: int, key: int) -> int | None:
    return indic[key] if len(indic := [it for it, x in enumerate(elements) if x == value]) > 0 else None
...
def get_first_three_index(elements: list[int]) -> int | None:
    ...
    """
    return get_indices(elements, 3, 0)
...
def get_last_three_index(elements: list[int]) -> int | None:
    ...
    """
    return get_indices(elements, 3, -1)
...
def get_by_index(elements: list[int], i: int, boundary: int) -> int | None:
    ...
    """
    return ret if (ret := elements[i]) > boundary else None
